...
in_features = 5#4 # 5 con inventario dell'altro agente
...
STEP = 15*5#100#15#15*5 # abbassa la epsilon ogni 100 azioni compiute
NUMIT = 5_000#10_000#1_000#5_000 # traiettorie di train, quelle di test sono sempre la meta', il numero totale delle azioni compiute in train sarà NUMIT x PASSI
EN = 10 # numero di esecuzioni

MIN_P = 9.8#9.8  #prezzo minimo
MAX_P = 10.2#10.2 #prezzo massimo
LR = 0.001 # learning rate
BATCH = 64 #8#128# batch size
...
class DQN(nn.Module):
    ...
    def __init__(self, in_size, hidden_layers_size):
        ...
        for _ in range(NUM_LAYERS):  # Adding 5 hidden layers
...
class Ambiente():
    ...
    def inventory_action_transform(self, q, x):
        ...
        q = r_tilde * m.cos(theta)
        x = r_tilde * m.sin(theta)
    ...
    def normalise(self, inventory, inventory_altro, time, price, x, min_p, max_p):
        '''
        performs the normalisation in the range [-1,1] for the feature of the NN
        '''
        q, x = self.inventory_action_transform(inventory, x)
        q_p = 2 * (inventory_altro - 0)/ INV - 1#, _ = self.inventory_action_transform(inventory_altro, x)
...
class ReplayMemory():
    ...
    def add(self, inv, time, p, inv_altro, x, next_inv, next_time, next_price, next_inv_altro, reward):
        
        self.memory.append([inv, time, p, inv_altro, x, next_inv, next_time, next_price, next_inv_altro, reward])
...
class Agente():
    ...
    def eval_Q(self, state, act, p_min, p_max, type = 'tensor', net = 'main'):
        ...
        if type == 'scalar' :
            ...
            q, t, p, x, q_p = Ambiente().normalise(state[0], state[1], state[2], state[3],act, p_min, p_max)
            in_features = torch.tensor([q, t, p, x, q_p], dtype=torch.float)
        ...
        if type == 'tensor':
            ...
            for i in range(len(state)):
                ...
                q, t, p, x, q_p = Ambiente().normalise(state[i][0], state[i][1], state[i][2], state[i][3], act[i], p_min, p_max)
                features.append(torch.tensor([q, t, p, x, q_p], dtype=torch.float))
    ...
    def q_action(self, state, min_p, max_p):
        ...
        with torch.no_grad():
            ...
            for i in range(int(abs(state[0]) + 1)):

                q, t, p, x, q_p = Ambiente().normalise(state[0], state[1], state[2], state[3], i, min_p, max_p) 
                features.append(torch.tensor([q, t, p, x, q_p], dtype=torch.float))
    ...
    def action(self, state, min_p, max_p):
       ...
       if state[0] <= 0:
           action = state[0] # 0 changed to state[0]

       el #elif state[1] != (PASSI - 1) and state[0] <= 0:
       #    action = state[0] ###### changed from 0 to state[0]
       '''
       if np.random.rand() <= self.epsilon and state[1] < (PASSI - 1):#
           n = state[0]
           p = 1 / (self.time_subdivisions - state[1])
           action = np.random.normal(loc= n/(PASSI - state[1]), scale=abs(n/(PASSI - state[1]))) #np.random.binomial(n, p)    
           
       elif state[1] >= (PASSI - 1):# and state[0] <= 0:#### changed from 0 to 0.0 and from >= to <=
           action = state[0]
    ...
    def train_1(self, transitions, data, p_min, p_max):
        ...
        '''
        state       = [tup[:4] for tup in transitions ]
        act         = [tup[4] for tup in transitions  ]
        next_state  = [tup[5:9] for tup in transitions]
        reward      = [tup[9] for tup in transitions  ]
    ...
    def test(self, inv, inv_altro, tempo, dati, p_min, p_max):
        '''
        does the testing on new data using the weights of the already trained NN
        '''
        state = [inv, tempo, dati[0], inv_altro]
    ...
    def step(self, inv, inv_altro, tempo, data, p_min, p_max):
        '''
        ...
        '''
        self.timestep += 1
        state = [inv, tempo, data[0], inv_altro]

        x = self.action(state, p_min, p_max)
        r = self.reward(inv, x, data)
        new_inv = inv - x
        next_state = [new_inv, tempo + 1, data[-1], inv_altro]
        self.memory.add(state[0], state[1], state[2], state[3], x, next_state[0], next_state[1], next_state[2],  next_state[3], r)
...
if __name__ == '__main__': 
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in range(slices):
                ...
                for agent_index in agen:
                    ...
                    loss, grad, state, epsilon, new_inv, action, reward = age[agent_index].step(inv[agent_index][i][j], inv[1-agent_index][i][j], tempo, dati, p_min, p_max)
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in range(slices):
                ...
                for agent_index in agen:
                    ...
                    (new_inv, x, reward) = age[agent_index].test(inv[agent_index][i][j], inv[1-agent_index][i][j], tempo, dati, p_min, p_max)
                    inv[agent_index][i+1][j] = new_inv
                    inve = new_inv + x
    ...
    np.save('C:/Users/macri/Desktop/ennesima/ql_tot', ql_tot)
    np.save('C:/Users/macri/Desktop/ennesima/ac_tot', ac_tot)
    np.save('C:/Users/macri/Desktop/ennesima/azioni_tot', azioni_tot)
    np.save('C:/Users/macri/Desktop/ennesima/stati_tot', states)
    np.save('C:/Users/macri/Desktop/ennesima/dati_tot', dati_tot)
    np.save('C:/Users/macri/Desktop/ennesima/re_tot', re_tot)
